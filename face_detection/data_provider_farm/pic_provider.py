...
from data_provider_farm.text_list_adapter import TextListAdapter
...
class PickleProvider(ProviderBaseclass):
    ...
    def read_by_index(self, index):
        ...
        frame = cv2.imdecode(im_buf, cv2.IMREAD_COLOR)
        # if flag==0:
        #     cv2.imwrite('fuyangben/'+str(index)+'.jpg',frame)
            # for box in bboxes:
            #     cv2.rectangle(frame, (int(box[0]), int(box[1])), (int(box[0]+box[2]), int(box[1]+box[3])), (0, 255, 0), 2)
            # cv2.imshow('sdf',frame)
            # cv2.waitKey()

        return frame, flag, bboxes
